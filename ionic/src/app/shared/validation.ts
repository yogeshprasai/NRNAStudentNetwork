...
export const Validation_Password_Reset_Send = {
    'email': [
            { type: 'required', message: 'Email is required.' },
            { type: 'pattern', message: 'Please enter a valid Email' },
            { type: 'noUserExist', message: 'No User Registered with above id' },
            { type: 'emailSendingFailed', message: 'Failure to send reset email, Please try again.' },
            { type: 'emailNotVerified', message: 'Email is not verified. Please contact us.' },
            { type: 'noMessageReceived', message: 'Server Error. Please try again.'}
        ]
}
